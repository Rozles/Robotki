...
visualization_msgs::MarkerArray marker_array_cylinders;
std::vector<int> cylinders_n;
...
cloud_cb (const pcl::PCLPointCloud2ConstPtr& cloud_blob)
{
  ...
  else
  {
      ...
      if (new_cylinder) {
        ...
        marker.ns = "cylinder";
        marker.id = marker_array_cylinders.markers.size();
        ...
        marker_array_cylinders.markers.push_back(marker);
        cylinders_n.push_back(1);
      }
  ...
  pubm.publish (marker_array_cylinders);
}
...
{
  // Initialize ROS
  ros::init (argc, argv, "cylinders");
  ros::NodeHandle nh;

  marker_array_cylinders = visualization_msgs::MarkerArray();
  ...
  pubm = nh.advertise<visualization_msgs::MarkerArray>("cylinders",1);
